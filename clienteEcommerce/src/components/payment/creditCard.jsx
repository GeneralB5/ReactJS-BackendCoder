function CreditCard({states}){
  
let word = states.EXP
let halfWord = Math.ceil(word.length / 2)
let newEXP =states.EXP >= 100 ? word.slice(0, halfWord) + "/" + word.slice(halfWord) : states.EXP
...
<div className="flip-card">
  <div className="flip-card-inner">
    <div className="flip-card-front">
      ...
      <p className="number">{states.NUMERODETARJETA > 0? states.NUMERODETARJETA >1000000000000000? states.NUMERODETARJETA.replace(/(.{4})/g, '$1-').slice(0 ,-1) : states.NUMERODETARJETA.replace(/(.{4})/g, '$1-') : "####-####-####-####"}</p>
      <p className="valid_thru">VALID THRU</p>
      <p className="date_8264">{states.EXP > 0 ? newEXP : "####"}</p>
      <p className="name" >{states.NOMBRE != ''? states.NOMBRE : "John Doe"}</p>
    </div>
  </div>
</div>
    )
