import { useContext, useState } from "react"
import wavesPattCreate from "../assets/wavesPattCreate.png"
import FormCreatePage from "../components/createPageContent/formCreate.jsx"
import { userInfoContext } from "../context/context.jsx"
function CreateProductPage(){
    const {user} = useContext(userInfoContext)    
    const [select,setSelect] = useState(null)
    console.log(select)
    const handleSubmitCreate =async(e)=>{
        e.preventDefault()
        if(select == ''){
            console.log("select no util")
            return;
        }
        const form = e.target
        const formData = new FormData(form)

        const newFormData = new FormData()

        const formJson = Object.fromEntries(formData.entries())
        const send = JSON.stringify({
            title:formJson.title,
            price:formJson.price,
            stock:formJson.stock,
            description:formJson.description,
            status:formJson.status,
            code:formJson.code,
            category:select
        })    
        newFormData.append('prod',send)
        newFormData.append('product',formJson.thumbnail)
         const data = await fetch(`http://localhost:8080/api/productos/`
         ,{
             method:"POST",
             credentials: 'include',
             body:newFormData
         })
         const dataJson = await data.json()
         if(dataJson.status == 'success'){window.location.href = 'http://localhost:5173/productos'}
    }
    return(
    <div style={{fontFamily:'system-ui',padding:'20px ',
    backgroundImage:`url(${wavesPattCreate})`,
    backgroundSize:'cover',
    backgroundRepeat:"no-repeat",
    backgroundPosition:"center"
    }}>
        <div style={{display:"flex",justifyContent:"center",alignItems:"center",flexDirection:"column",gap:15}}>
            <div style={{
                width:150,
                height:150,
                borderRadius:"50%",
                background: '#e0e0e0',
                boxShadow:'20px 20px 47px #b1b1b1,-20px -20px 47px #ffffff',
                backgroundImage:`url(${user!=undefined? user.thumbnail : "data:image/png;base64," })`,
                backgroundSize:'contain',
                backgroundRepeat:'no-repeat'
            }} >

            </div>
            <h2>{user != undefined? user.first_name : "here goes your name"}</h2>
        </div>
        <FormCreatePage onSubmit={handleSubmitCreate} setSelect={setSelect} />
        
    </div>
    )
}
export default CreateProductPage